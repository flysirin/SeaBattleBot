...
async def on_startup(dispatcher: Dispatcher):
    await dispatcher.storage.set_data(bot=bot_obj, key=bot_storage_key, data={"cur_session_ids": set()})


async def on_shutdown(dispatcher: Dispatcher):
    data = await dispatcher.storage.get_data(bot=bot_obj, key=bot_storage_key)
